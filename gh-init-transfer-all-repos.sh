...
echo "Requirements: gh auth login for <github-username> organization (step #5 below)"
echo "This script initilizes a transfer ALL (public + private) repositories in the <github-username> organization"
echo "It shows the visibility of each initialized repository transfer"
...
if [ $# -ne 2 ]; then
    echo "Usage: $0 <gitub-username> <target_github-username>"
    exit 1
fi
...
# List all repos and initialize transfer for each
gh repo list "$USERNAME" --limit 100 --json name,visibility --jq '.[] | .name' | while read repo; do
...
# List repos with transfer requests initialized
echo "Repos with transfer requests initialized for $TARGET_USER:"
